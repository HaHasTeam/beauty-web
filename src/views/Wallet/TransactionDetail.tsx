...
const TransactionItem = ({ transaction }: { transaction: ITransaction }) => {
  ...
  const getTransactionIcon = () => {
    switch(transaction.type) {
      ...
      case TransactionTypeEnum.ORDER_PURCHASE:
      case TransactionTypeEnum.BOOKING_PURCHASE:
        return <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>;
      case TransactionTypeEnum.ORDER_REFUND:
      case TransactionTypeEnum.BOOKING_REFUND:
        return <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 15v-1a4 4 0 00-4-4H8m0 0l3 3m-3-3l3-3m9 14V5a2 2 0 00-2-2H6a2 2 0 00-2 2v16l4-2 4 2 4-2 4 2z" /></svg>;
      case TransactionTypeEnum.ORDER_CANCEL:
      case TransactionTypeEnum.BOOKING_CANCEL:
        return <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>;
      default:
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mt-2 bg-white/60 backdrop-blur-sm rounded-lg p-3 border border-slate-100/70">
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2 mb-1">
            <svg className={`w-3.5 h-3.5 ${accentColors.balanceIcon}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
            </svg>
            <span className="text-xs font-medium text-slate-700">{t('wallet.transaction.balanceAfter', 'Số dư sau giao dịch')}</span>
          </div>
          <span className={`font-semibold ${accentColors.balance} text-base`}>{formatCurrency(transaction.balanceAfterTransaction)}</span>
        </div>
        
        {transaction.paymentMethod && (
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-2 mb-1">
              <svg className={`w-3.5 h-3.5 ${accentColors.paymentIcon}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z" />
              </svg>
              <span className="text-xs font-medium text-slate-700">{t('wallet.transaction.paymentMethod', 'Phương thức thanh toán')}</span>
            </div>
            <span className="font-semibold text-slate-900 text-base capitalize">{getTranslatedPaymentMethod(transaction.paymentMethod)}</span>
          </div>
        )}
        ...
        {transaction.order && (
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-2 mb-1">
              <svg className={`w-3.5 h-3.5 ${accentColors.orderIcon}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
              </svg>
              <span className="text-xs font-medium text-slate-700">{t('wallet.transaction.relatedOrder', 'Đơn hàng liên quan')}</span>
            </div>
            <div className="flex items-center gap-2">
              ...
              <Button 
                variant="link" 
                className="h-auto p-0 text-primary font-medium text-xs"
                asChild
              >
      ...
      {transaction.description && (
        <div className="mt-2 text-sm bg-white rounded-lg p-2 shadow-sm border border-slate-100 hover:shadow-md transition-shadow duration-200">
          <div className="flex items-center gap-1.5 mb-0.5 text-muted-foreground text-xs font-medium">
            <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
...
const MetricCard = ({ 
  ...
  return (
    <Card className="bg-white/80 backdrop-blur-sm rounded-xl shadow-md border border-border hover:shadow-lg transition-all duration-300 overflow-hidden group">
      <div className="absolute w-32 h-32 -right-12 -top-12 rounded-full opacity-10 transform group-hover:scale-110 transition-transform duration-300" style={{ backgroundColor: valueColor.replace('text-', '').includes('primary') ? 'var(--primary)' : `var(--${richBgColor.replace('bg-', '').replace('-100', '-200')})` }}></div>
      ...
      <CardContent className="p-4 relative">
        <div className="flex items-center gap-3 mb-2">
          <div className={`flex items-center justify-center w-10 h-10 rounded-lg ${richBgColor} text-slate-900 shadow-sm`}>
            {icon}
          </div>
          <p className="text-muted-foreground font-medium text-sm">{label}</p>
        </div>
        <p className={`${valueColor} font-bold text-2xl tracking-tight`}>
          {value}
        </p>
      </CardContent>
    </Card>
    ...
    if (transactionList.length === 0) {
      return (
        <div className="py-6 text-center bg-white/60 rounded-lg border border-slate-100 shadow-sm">
          ...
          <p className="text-slate-700 font-medium">{t('wallet.transaction.noTransactions', 'Không có giao dịch nào')}</p>
          <p className="text-xs text-muted-foreground mt-0.5">{t('wallet.transaction.tryDifferentFilter', 'Hãy thử bộ lọc khác')}</p>
        </div>
      )
    }
            ...
            <PaginationContent className="bg-white rounded-lg px-2 py-1 shadow-sm border border-slate-100 flex flex-wrap gap-1 justify-center">
              ...
              <PaginationItem>
                <PaginationNext 
                  ...
                  className={cn(
                    "h-8 min-w-[32px] p-0 flex items-center justify-center",
                    currentPageNumber >= totalPages - 1 ? "pointer-events-none opacity-50" : "cursor-pointer hover:bg-muted/20"
                  )}
                />
              </PaginationItem>
            </PaginationContent>
        ...
        <Card className="w-full bg-white/80 backdrop-blur-sm rounded-xl shadow-md border border-border overflow-hidden">
          <CardHeader className="pb-1 pt-3 px-4">
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="font-medium text-base">{t('wallet.transaction.recentTransactions', 'Giao dịch gần đây')}</CardTitle>
            </div>
          </CardHeader>
          <CardContent className="p-3 pt-2">
            <Tabs defaultValue={selectedTab || 'all'} onValueChange={handleTabChange} className="w-full">
              <TabsList className="flex flex-wrap h-auto p-1 mb-3 bg-gradient-to-r from-slate-100/90 to-indigo-50/50 rounded-lg">
                <TabsTrigger value="all" className="text-xs whitespace-nowrap h-7 px-3 data-[state=active]:shadow-md data-[state=active]:bg-white rounded-md">
                  {t('wallet.transaction.tabs.all', 'Tất cả')}
                </TabsTrigger>
                <TabsTrigger value="deposit" className="text-xs whitespace-nowrap h-7 px-3 data-[state=active]:shadow-md data-[state=active]:bg-white rounded-md">
                  {t('wallet.transaction.tabs.deposit', 'Nạp tiền')}
                </TabsTrigger>
                <TabsTrigger value="withdraw" className="text-xs whitespace-nowrap h-7 px-3 data-[state=active]:shadow-md data-[state=active]:bg-white rounded-md">
                  {t('wallet.transaction.tabs.withdraw', 'Rút tiền')}
                </TabsTrigger>
                <TabsTrigger value="orderPurchase" className="text-xs whitespace-nowrap h-7 px-3 data-[state=active]:shadow-md data-[state=active]:bg-white rounded-md">
                  {t('wallet.transaction.tabs.orderPurchase', 'Mua hàng')}
                </TabsTrigger>
                <TabsTrigger value="bookingPurchase" className="text-xs whitespace-nowrap h-7 px-3 data-[state=active]:shadow-md data-[state=active]:bg-white rounded-md">
                  {t('wallet.transaction.tabs.bookingPurchase', 'Đặt lịch')}
                </TabsTrigger>
                <TabsTrigger value="orderRefund" className="text-xs whitespace-nowrap h-7 px-3 data-[state=active]:shadow-md data-[state=active]:bg-white rounded-md">
                  {t('wallet.transaction.tabs.orderRefund', 'Hoàn tiền đơn hàng')}
                </TabsTrigger>
                <TabsTrigger value="bookingRefund" className="text-xs whitespace-nowrap h-7 px-3 data-[state=active]:shadow-md data-[state=active]:bg-white rounded-md">
                  {t('wallet.transaction.tabs.bookingRefund', 'Hoàn tiền đặt lịch')}
                </TabsTrigger>
                <TabsTrigger value="orderCancel" className="text-xs whitespace-nowrap h-7 px-3 data-[state=active]:shadow-md data-[state=active]:bg-white rounded-md">
                  {t('wallet.transaction.tabs.orderCancel', 'Hủy đơn hàng')}
                </TabsTrigger>
                <TabsTrigger value="bookingCancel" className="text-xs whitespace-nowrap h-7 px-3 data-[state=active]:shadow-md data-[state=active]:bg-white rounded-md">
