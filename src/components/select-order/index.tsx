...
const getOrderItemDisplay = (order: IOrder) => {
  ...
  const formattedDate = extendedOrder?.createdAt 
    ? format(new Date(extendedOrder.createdAt), 'dd MMM yyyy') 
    : 'N/A'
  ...
  const total = typeof extendedOrder.totalPrice === 'number' 
    ? extendedOrder.totalPrice 
    : 0
  ...
  return (
    <div className="flex items-center gap-2 py-1 w-full">
      ...
      {/* Status Badge */}
      <Badge className={`${getStatusColor(status)} text-white text-xs px-1.5 py-0.5 h-5`}>
        {status}
      </Badge>
      ...
      filterOption={(option, inputValue) => {
        ...
        return searchTerms.every(term => {
          ...
          const formattedDate = order.createdAt 
            ? format(new Date(order.createdAt), 'dd MMM yyyy').toLowerCase()
            : '';
