...
import type { ExtendedColumnSort, ExtendedSortingState, StringKeyOf } from '@/types/table'
...
export function DataTableSortList<TData>({ table, debounceMs, shallow }: DataTableSortListProps<TData>) {
  ...
  function addSort() {
    const firstAvailableColumn = sortableColumns.find((column) => !sorting.some((s) => s.id === column.id))
  ...
  function updateSort({
    ...
    updateFunction((prevSorting) => {
      ...
      const updatedSorting = prevSorting.map((sort) => (sort.id === id ? { ...sort, ...field } : sort))
