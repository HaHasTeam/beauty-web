        ...
        <CardContent className="pt-12 pb-8 text-center relative overflow-hidden">
          ...
          <div className="absolute inset-0 pointer-events-none">
            {[...Array(12)].map((_, i) => (
              <span
                key={i}
                className={`absolute w-3 h-3 bg-${colorClasses[color]}-400 rounded-full animate-float`}
                style={{
          ...
          <div className="relative mb-6">
            {status === ResultEnum.SUCCESS ? (
              <div
                className={`w-20 h-20 bg-${colorClasses[color]}-500 rounded-full mx-auto flex items-center justify-center`}
              >
                <Check className="w-10 h-10 text-white" />
              </div>
            ) : (
              <div className="w-20 h-20 rounded-full mx-auto flex items-center justify-center">
                <Frown className={`w-20 h-20 text-${colorClasses[color]}-500`} />
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <Button
              className={`bg-${colorClasses[color]}-500 hover:bg-${colorClasses[color]}-600`}
