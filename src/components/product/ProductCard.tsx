      ...
      <CardFooter className="flex flex-col gap-3 p-4">
        <div className="flex justify-between items-center w-full">
          <div className="flex items-baseline gap-2">
            <span className="text-xl font-semibold">{t('product.currentPrice', { price: product?.currentPrice })}</span>
          ...
          {product?.deal && product?.deal > 0 && (
            <span className="text-red-500 font-medium">-{(product?.deal * 100).toFixed(0)}%</span>
          )}
        </div>
        <div className="w-full space-y-1.5">
          <SoldProgress soldAmount={product?.soldAmount ?? 0} maxAmount={product?.productAmount} />
        </div>
